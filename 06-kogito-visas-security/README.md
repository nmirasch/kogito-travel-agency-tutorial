...
will add additional handling to the approved or rejected visa applications.
...
```
docker run -e KEYCLOAK_USER=admin -e KEYCLOAK_PASSWORD=admin  -e KEYCLOAK_IMPORT=/tmp/kogito-realm.json -v <kogito-travel-agency-tutorial_absolute_path>/06-kogito-visas-security/config/kogito-realm.json:/tmp/kogito-realm.json -p 8280:8080  jboss/keycloak
```
...
Note: This setup is required just once, this service can reuse the keycloak setup performed at **kogito-travel-agency**  
...
```
java -jar  -Dkogito.protobuf.folder={full path to proto files folder} -Dquarkus.profile=keycloak data-index-service-0.4.0-runner.jar 
```
...
Once the service is up and running, you can use the following examples to interact with the **Kogito Visas Service**.
...
```sh
curl -H "Authorization: Bearer "$access_token -X GET http://localhost:8090/visaApplications
```
...
```sh
curl -H "Authorization: Bearer "$access_token -X GET http://localhost:8090/visaApplications/{uuid}
```
...
```sh
curl -H "Authorization: Bearer "$access_token -X DELETE http://localhost:8090/visaApplications/{uuid}
```
...
```sh
curl -H "Authorization: Bearer "$access_token -X GET http://localhost:8090/visaApplications/{uuid}/tasks
```
...
```sh
curl -H "Authorization: Bearer "$access_token -X GET http://localhost:8090/visaApplications/{uuid}/ApplicationApproval/{task-uuid}
```
...
```sh
curl -H "Authorization: Bearer "$access_token -H "Content-Type: application/json" -H "Accept: application/json" -X POST http://localhost:8090/visaApplications/{uuid}/ApplicationApproval/{task-uuid} -d @- << EOF
{
	"application" : {
		"firstName" : "Jan",
		"lastName" : "Kowalski",
