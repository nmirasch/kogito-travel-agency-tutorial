...
Adding security layer over **Kogito Travel Agency** and **Kogito Data Index** services interactions, using Keycloak as Open Id 
...
* Add security check on **Kogito Travel Agency** and **Kogito Data Index Service**, relaying authentication on keycloak 
...
```
docker run -e KEYCLOAK_USER=admin -e KEYCLOAK_PASSWORD=admin  -e KEYCLOAK_IMPORT=/tmp/kogito-realm.json -v <kogito-travel-agency-tutorial_absolute_path>/06-kogito-travel-agency-security/config/kogito-realm.json:/tmp/kogito-realm.json -p 8280:8080  jboss/keycloak
```
...
This service works with .proto files that define the data model. Once **Kogito Travel Service** is started,
 /target/classes/persistence/travels.proto is generated and it has to be copied to the new proto files folder.

To start the **Kogito Data Index Service** with security enabled (using keycloak quarkus profile -Dquarkus.profile=keycloak), 
just passing the full path of the proto files folder and execute  

```
java -jar  -Dkogito.protobuf.folder={full path to proto files folder} -Dquarkus.profile=keycloak data-index-service-0.7.0-runner.jar 
```
...
The app will redirect to keycloak login page. jdoe/jdoe can be used there regarding it comes defined by keycloak 
config provided.
...
The application is using bearer token authorization and the first thing to do is to obtain an access token from the Keycloak 
...
```sh
curl -H "Content-Type: application/json" -H "Accept: application/json" -X POST http://localhost:8080/travels -H "Authorization: Bearer "$access_token -d @- << EOF
{
	"traveller" : {
		"firstName" : "John",
		"lastName" : "Doe",
...
```sh
curl -X POST http://localhost:8080/travels -H "Content-Type: application/json" -H "Accept: application/json" -H "Authorization: Bearer "$access_token -d @- << EOF
{
	"traveller" : {
		"firstName" : "Jan",
		"lastName" : "Kowalski",
...
```sh
curl -X GET http://localhost:8080/travels -H "Authorization: Bearer "$access_token
```
...
```sh
curl -X GET http://localhost:8080/travels/{uuid} -H "Authorization: Bearer "$access_token
```
...
```sh
curl -X DELETE http://localhost:8080/travels/{uuid} -H "Authorization: Bearer "$access_token
```
...
```sh
curl -X GET http://localhost:8080/travels/{uuid}/tasks -H "Authorization: Bearer "$access_token
```
...
```sh
curl -X GET http://localhost:8080/travels/{uuid}/VisaApplication/{task-uuid} -H "Authorization: Bearer "$access_token
```
...
```sh
curl -H "Content-Type: application/json" -H "Accept: application/json" -H "Authorization: Bearer "$access_token -X POST http://localhost:8080/travels/{uuid}/VisaApplication/{task-uuid} -d @- << EOF
{
	"visaApplication" : {
		"firstName" : "Jan",
		"lastName" : "Kowalski",
...
```sh
curl -X GET http://localhost:8080/travels/{uuid}/ConfirmTravel/{task-uuid} -H "Authorization: Bearer "$access_token
```
...
```sh
curl -H "Content-Type: application/json" -H "Accept: application/json" -H "Authorization: Bearer "$access_token-X POST http://localhost:8080/travels/{uuid}/ConfirmTravel/{task-uuid} -d '{}'
```
